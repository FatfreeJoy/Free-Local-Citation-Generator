(function (root, factory) {
  ...
  const STYLE_REGISTRY = Object.freeze({
    apa7: Object.freeze({ id: "apa7", label: "APA 7th edition", available: true }),
    mla9: Object.freeze({ id: "mla9", label: "MLA 9th edition", available: false }),
    chicago17: Object.freeze({ id: "chicago17", label: "Chicago 17th edition", available: false }),
    harvard: Object.freeze({ id: "harvard", label: "Harvard", available: false })
  });
  ...
  function formatEditors(names) { return joinNames(names || [], personEditor, false); }
  ...
  function parseBibTeX(input) {
    ...
    return rawEntries.map(({ rawType, fields }) => {
      ...
      return normalizeRecord({
        ...
        publisher: f.publisher, institution: f.institution || f.school,
  ...
  function parseRIS(input) {
    ...
    return entries.map((e) => {
      ...
      return normalizeRecord({
        ...
        publisher: one(e, ["PB"]), institution: one(e, ["IN", "A3"]),
        edition: one(e, ["ET"]), reportNumber: one(e, ["SN"]),
        thesisType, repository: one(e, ["DB"]), event: one(e, ["T3"]), note
      });
    });
  }
  ...
  function formatRecord(input) {
    ...
    return { record, text: text.filter(Boolean).join(" ").replace(/\s+/g, " ").trim(), html: html.filter(Boolean).join(" ").replace(/\s+/g, " ").trim() };
  }

  function sortKey(record) {
    const lead = formatAuthors(record.authors || []) || record.title;
  ...
  function formatRecords(records, styleId) {
    ...
    return normalized.map(formatRecord);
  }
  ...
  return {
    ...
    formatRecord, formatRecords, escapeHtml,

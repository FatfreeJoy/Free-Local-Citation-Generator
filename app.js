(function () {
  ...
  const STORAGE_KEY = "reference-studio.workspace";
  const BACKUP_PRODUCT = "Reference Studio";
  ...
  const referenceMetaDialog = $("#reference-meta-dialog");
  ...
  let referenceMetaTarget = null;
  let dragPayload = null;
  let authorSequence = 0;
  let pendingImportedWorkspace = null;
  ...
  const TAG_COLOR_IDS = new Set(TAG_COLORS.map(([id]) => id));
  const TAG_COLOR_LABELS = new Map(TAG_COLORS);
  const EDITABLE_REFERENCE_TYPES = new Set(["article", "web", "book", "chapter"]);

  function normalizeWorkspaceRecord(record) {
    const normalized = core.normalizeRecord(record);
    normalized.tagColor = TAG_COLOR_IDS.has(normalized.tagColor) ? normalized.tagColor : "";
    normalized.userNote = normalized.userNote.slice(0, MAX_NOTE_LENGTH);
    return normalized;
  }

  function normalizeWorkspace(value) {
    return workspaceCore.normalizeWorkspace(value, core.styles, normalizeWorkspaceRecord);
  }
  ...
  function showToast(message, action) {
    ...
    toastTimer = window.setTimeout(() => toast.classList.remove("is-visible"), action ? 8000 : 2200);
  }
  ...
  function renderCitations() {
    ...
    state.formatted.forEach((citation, index) => {
      ...
      if (TAG_COLOR_IDS.has(citation.record.tagColor)) item.dataset.tagColor = citation.record.tagColor;
      ...
      if (parenthetical) {
        ...
        inTextLabel.textContent = "In-text";
        ...
        ["parenthetical", "narrative"].forEach((mode) => {
          ...
          const inTextValue = core.formatInText(citation.record, list.style, mode, "");
          preview.textContent = inTextValue;
          preview.title = inTextValue;
      ...
      const annotation = makeButton("", "citation-action annotation-action", "edit-reference-meta", { recordId: citation.record.id });
      if (TAG_COLOR_IDS.has(citation.record.tagColor)) {
        annotation.dataset.tagColor = citation.record.tagColor;
      ...
      const colorLabel = TAG_COLOR_LABELS.get(citation.record.tagColor) || "";
      annotationLabel.textContent = citation.record.userNote && colorLabel ? `${colorLabel} · Note` : citation.record.userNote ? "Note" : colorLabel || "Tag / note";
      annotation.appendChild(annotationLabel);
      annotation.setAttribute("aria-label", `${citation.record.userNote || colorLabel ? "Edit" : "Add"} tag or note for reference ${index + 1}${colorLabel ? `; current color ${colorLabel}` : ""}`);
      const annotationTitle = [colorLabel ? `Tag: ${colorLabel}` : "", citation.record.userNote ? `Note: ${citation.record.userNote.slice(0, 180)}` : ""].filter(Boolean).join("\n");
      if (annotationTitle) annotation.title = annotationTitle;
      if (EDITABLE_REFERENCE_TYPES.has(citation.record.type)) {
        const edit = makeButton("Edit source", "citation-action", "edit-reference", { recordId: citation.record.id });
        edit.setAttribute("aria-label", `Edit source details for reference ${index + 1}`);
      ...
      actions.append(annotation, copy, remove);
  ...
  function addFormattedReferences() {
    ...
    const result = appendRecords(records);
    if (result.added) formattedData.value = "";
  ...
  async function readFiles(fileList) {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_IMPORT_FILE_BYTES) { errors.push(`${file.name}: larger than 25 MB`); continue; }
      try {
  ...
  function countWorkspaceItems(workspace) {
    const projects = workspace.projects.length;
    const lists = workspace.projects.reduce((total, project) => total + project.lists.length, 0);
    const references = workspace.projects.reduce((total, project) => total + project.lists.reduce((sum, list) => sum + list.records.length, 0), 0);
    return { projects, lists, references };
  }
  ...
  function exportWorkspaceBackup() {
    ...
    link.href = url;
    link.download = `reference-studio-backup-${date}.json`;
  ...
  async function prepareWorkspaceRestore(file) {
    ...
    if (file.size > MAX_BACKUP_BYTES) {
      showToast("That backup is larger than 10 MB");
      return;
    }
    try {
      const parsed = JSON.parse(await readFileText(file));
      if (!parsed || parsed.product !== BACKUP_PRODUCT || parsed.formatVersion !== BACKUP_FORMAT_VERSION || !parsed.workspace) {
        throw new Error("Choose a current Reference Studio backup file.");
      }
      pendingImportedWorkspace = normalizeWorkspace(parsed.workspace);
      askConfirmation("restore-backup", null, null);
    } catch (error) {
      pendingImportedWorkspace = null;
      showToast(error.message || "The backup could not be restored");
    }
  }
  ...
  function syncNameRows(kind) {
    ...
    rows.forEach((row, index) => {
      ...
      last.setAttribute("aria-label", `${role} ${index + 1} last name or group`);
  ...
  function addNameRow(kind, value, shouldFocus) {
    ...
    lastLabel.textContent = "Last name or group";
    ...
    last.placeholder = "Miller or WHO";
  ...
  function setNameRows(kind, people) {
    $(`#${kind}-rows`).replaceChildren();
    const values = Array.isArray(people) && people.length ? people : [null];
    values.forEach((person) => addNameRow(kind, person, false));
  }

  function clearManualEditState(resetForm) {
    manualEditTarget = null;
    $("#manual-submit-label").textContent = "Add reference";
    $("#manual-cancel-edit-button").hidden = true;
  ...
  function startManualEdit(recordId) {
    const project = currentProject();
    const list = currentList();
    const record = list.records.find((item) => item.id === recordId);
    if (!record || !EDITABLE_REFERENCE_TYPES.has(record.type)) {
      showToast("This reference type cannot be edited in the manual form");
      return;
    }
    manualEditTarget = { projectId: project.id, listId: list.id, recordId };
    const form = $("#manual-form");
    form.reset();
    $("#reference-type").value = record.type;
    setNameRows("author", record.authors);
    setNameRows("editor", record.editors);
    updateReferenceForm();
    $("#year").value = record.year || "";
    $("#month").value = record.month || "";
    $("#day").value = record.day || "";
    $("#title").value = record.title || "";
    $("#journal").value = record.containerTitle || "";
    $("#volume").value = record.volume || "";
    $("#issue").value = record.issue || "";
    $("#pages").value = record.pages || "";
    $("#edition").value = record.edition || "";
    $("#publisher").value = record.publisher || "";
    $("#place").value = record.place || "";
    $("#accessed").value = record.accessed || "";
    $("#doi-url").value = record.doi ? `https://doi.org/${record.doi}` : (record.url || "");
    $("#manual-submit-label").textContent = "Save changes";
    $("#manual-cancel-edit-button").hidden = false;
    setMessage(manualMessage, "");
    switchTab("manual");
    $("#manual-panel").scrollIntoView({ behavior: "smooth", block: "start" });
    window.setTimeout(() => $("#title").focus(), 0);
  }

  function manualRecordFromForm(existingRecord) {
    const type = $("#reference-type").value;
    const value = (selector) => $(selector).value.trim();
    const doiOrUrl = value("#doi-url");
    return normalizeWorkspaceRecord({
      id: existingRecord && existingRecord.id,
      key: existingRecord && existingRecord.key,
      source: "manual",
      type,
      authors: collectNames("author"),
      editors: type === "chapter" ? collectNames("editor") : [],
      year: value("#year"),
      month: type === "web" ? value("#month") : "",
      day: type === "web" ? value("#day") : "",
      date: value("#year"),
      title: value("#title"),
      containerTitle: type === "book" ? "" : value("#journal"),
      volume: type === "article" ? value("#volume") : "",
      issue: type === "article" ? value("#issue") : "",
      pages: ["article", "chapter"].includes(type) ? value("#pages") : "",
      articleNumber: type === "article" && existingRecord && existingRecord.type === "article" ? existingRecord.articleNumber : "",
      edition: ["book", "chapter"].includes(type) ? value("#edition") : "",
      publisher: ["book", "chapter"].includes(type) ? value("#publisher") : "",
      place: ["book", "chapter"].includes(type) ? value("#place") : "",
      accessed: ["article", "web"].includes(type) ? value("#accessed") : "",
      doi: /^\s*(?:doi\s*:|https?:\/\/(?:dx\.)?doi\.org\/|10\.)/i.test(doiOrUrl) ? doiOrUrl : "",
      url: doiOrUrl,
      note: existingRecord && existingRecord.note,
      tagColor: existingRecord && existingRecord.tagColor,
      userNote: existingRecord && existingRecord.userNote
    });
  }
  ...
  function transferOrganizedList(copyOnly) {
    ...
    const result = copyOnly
      ? workspaceCore.duplicateListTo(state.workspace, organizeTarget.projectId, organizeTarget.listId, destinationId, normalizeWorkspaceRecord)
  ...
  function renderTagColorOptions() {
    const options = $("#tag-color-options");
    options.replaceChildren();
    TAG_COLORS.forEach(([id, label]) => {
      ...
      swatch.dataset.tagColor = id;
      swatch.setAttribute("aria-hidden", "true");
      const name = document.createElement("span");
      name.className = "tag-color-name";
  ...
  function openReferenceMeta(recordId) {
    const project = currentProject();
    const list = currentList();
    const record = list.records.find((item) => item.id === recordId);
    if (!record) return;
    referenceMetaTarget = { projectId: project.id, listId: list.id, recordId };
    $("#reference-meta-context").textContent = (record.title || record.formattedText || "Untitled reference").slice(0, 180);
    $("#reference-note").value = record.userNote || "";
    $$('input[name="tagColor"]').forEach((input) => { input.checked = input.value === record.tagColor; });
    openDialog(referenceMetaDialog);
    window.setTimeout(() => $("#reference-note").focus(), 0);
  }

  function saveReferenceMeta() {
    if (!referenceMetaTarget) return;
    const project = projectById(referenceMetaTarget.projectId);
    const list = listById(project, referenceMetaTarget.listId);
    const record = list && list.records.find((item) => item.id === referenceMetaTarget.recordId);
    if (!record) { closeDialog(referenceMetaDialog); return; }
    const selected = $('input[name="tagColor"]:checked');
    record.tagColor = selected && TAG_COLOR_IDS.has(selected.value) ? selected.value : "";
    record.userNote = $("#reference-note").value.trim().slice(0, MAX_NOTE_LENGTH);
    saveWorkspace();
    renderWorkspace();
    closeDialog(referenceMetaDialog);
    showToast("Reference details saved");
  }
  ...
  function askConfirmation(action, project, list) {
    ...
    if (action === "restore-backup" && pendingImportedWorkspace) {
      const counts = countWorkspaceItems(pendingImportedWorkspace);
      content = ["Replace the current workspace?", `The backup contains ${counts.projects} project${counts.projects === 1 ? "" : "s"}, ${counts.lists} list${counts.lists === 1 ? "" : "s"}, and ${counts.references} reference${counts.references === 1 ? "" : "s"}. Your current workspace will be replaced, and unsaved source-entry fields will be cleared.`];
    }
    if (!content) return;
  ...
  // Event wiring
  $("#mobile-projects-toggle").addEventListener("click", () => {
    const rail = $(".project-rail");
    const button = $("#mobile-projects-toggle");
    const expanded = rail.classList.toggle("is-open");
    button.setAttribute("aria-expanded", String(expanded));
    button.setAttribute("aria-label", `${expanded ? "Hide" : "Show"} projects and backups`);
  });

  $("#export-workspace-button").addEventListener("click", exportWorkspaceBackup);
  backupFileInput.addEventListener("change", (event) => prepareWorkspaceRestore(event.target.files && event.target.files[0]));
  $(".backup-file-label").addEventListener("keydown", (event) => {
  ...
  $("#manual-form").addEventListener("submit", (event) => {
    ...
    if (manualEditTarget) {
      const project = projectById(manualEditTarget.projectId);
      const list = listById(project, manualEditTarget.listId);
      const recordIndex = list ? list.records.findIndex((record) => record.id === manualEditTarget.recordId) : -1;
      if (!list || recordIndex < 0) {
        clearManualEditState(true);
        showToast("That reference is no longer available");
        return;
      }
      const currentRecord = list.records[recordIndex];
      const updatedRecord = manualRecordFromForm(currentRecord);
      const duplicate = list.records.some((record, index) => index !== recordIndex && recordFingerprint(record) === recordFingerprint(updatedRecord));
      if (duplicate) {
        setMessage(manualMessage, "Another reference in this list already has the same source details.");
        return;
      }
      list.records[recordIndex] = updatedRecord;
      saveWorkspace();
      renderWorkspace();
      clearManualEditState(true);
    ...
    setMessage(manualMessage, "");
    const record = manualRecordFromForm(null);
  ...
  $("#manual-cancel-edit-button").addEventListener("click", () => {
    clearManualEditState(true);
  ...
  projectTree.addEventListener("click", (event) => {
    ...
    if (button.dataset.action === "toggle-project") {
      ...
      if (manualEditTarget) clearManualEditState(true);
  ...
  $("#reference-meta-close-button").addEventListener("click", () => closeDialog(referenceMetaDialog));
  $("#reference-meta-cancel-button").addEventListener("click", () => closeDialog(referenceMetaDialog));
  $("#clear-tag-button").addEventListener("click", () => {
    $$('input[name="tagColor"]').forEach((input) => { input.checked = false; });
  });
  $("#reference-meta-form").addEventListener("submit", (event) => {
    event.preventDefault();
    saveReferenceMeta();
  });
  ...
  $("#confirm-button").addEventListener("click", () => {
    ...
    if (confirmAction.action === "restore-backup" && pendingImportedWorkspace) {
      state.workspace = pendingImportedWorkspace;
      pendingImportedWorkspace = null;
      resetEntryFields();
      saveWorkspace();
      renderWorkspace();
    ...
    if (manualEditTarget) {
      const editWasAffected = (confirmAction.action === "delete-project" && manualEditTarget.projectId === confirmAction.projectId)
        || (["delete-list", "clear-list"].includes(confirmAction.action)
          && manualEditTarget.projectId === confirmAction.projectId
          && manualEditTarget.listId === confirmAction.listId);
      if (editWasAffected) clearManualEditState(true);
    }
    saveWorkspace();
    renderWorkspace();
    closeDialog(confirmDialog);
  ...
  $("#confirm-cancel-button").addEventListener("click", () => {
    if (confirmAction && confirmAction.action === "restore-backup") pendingImportedWorkspace = null;
    closeDialog(confirmDialog);
  });
  ...
  citationList.addEventListener("input", (event) => {
    ...
    locator.closest(".intext-tools").querySelectorAll(".intext-option").forEach((button) => {
      const preview = button.querySelector("strong");
      const inTextValue = core.formatInText(citation.record, currentList().style, button.dataset.mode, locator.value);
      preview.textContent = inTextValue;
      preview.title = inTextValue;
    });
  });
  ...
  citationList.addEventListener("click", async (event) => {
    const button = event.target.closest("button[data-action]");
    if (!button) return;
    if (button.dataset.action === "edit-reference-meta") {
      openReferenceMeta(button.dataset.recordId);
      return;
    }
    if (button.dataset.action === "edit-reference") {
      startManualEdit(button.dataset.recordId);
      return;
    }
    if (button.dataset.action === "remove-reference") {
      const project = currentProject();
      const list = currentList();
      const recordIndex = list.records.findIndex((record) => record.id === button.dataset.recordId);
      if (recordIndex < 0) return;
      const removedRecord = list.records.splice(recordIndex, 1)[0];
      if (manualEditTarget && manualEditTarget.recordId === removedRecord.id) clearManualEditState(true);
      saveWorkspace();
      renderWorkspace();
      showToast("Reference removed", {
        label: "Undo",
        onClick: () => {
          const destinationProject = projectById(project.id);
          const destinationList = listById(destinationProject, list.id);
          if (!destinationList || destinationList.records.some((record) => record.id === removedRecord.id)) return;
          destinationList.records.splice(Math.min(recordIndex, destinationList.records.length), 0, removedRecord);
          saveWorkspace();
          renderWorkspace();
  ...
  $("#info-done-button").addEventListener("click", () => closeDialog(welcomeDialog));
  $("#welcome-close-button").addEventListener("click", () => closeDialog(welcomeDialog));
